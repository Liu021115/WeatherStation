...
int main(void)
{
  ...
  uart_init(115200);                													//4G串口初始化
	...
	M24LC64F_ReadReg(0x300,(uint8_t *)HttpVersion,32);      		//读取版本信息
	sprintf(HttpCmp,"%s_04",AdressID);
	if(strcmp(HttpVersion,HttpCmp)!=0)
		M24LC64F_WriteReg(0x300,(uint8_t *)HttpCmp,32);						//写入错误的处理！！！防止重复下载
...
//	
		fatfs_init();
		Fat_FilePath(Fatname.Filename,Fatname.Dirname,&RTCTime);
	Delay_ms(100);
	...
	while (1)
	{
		...
		if(StateOfHarfWare != CLOSE_ALL ){																//判断状态RS485未关闭
			switch(Timecount)  																							//功能周期执行  进行定期喂狗
			{
				...
				case 80: IWDG_ReloadCounter();break;				
			}
		}
		else{																				//判断状态RS485关闭
			if(Timecount %40 == 0){										
