...
uint8_t EEPROM_read(uint8_t *Version,uint8_t *MN){
	...
	uint8_t count = 0; 
	while(readflag){
		readflag = M24LC64F_ReadReg(0x300,Version,32);	 		//读取EEPROM存放的版本信息，读取长度32位
		Delay_ms(10);														//延时十毫秒，确保读取稳定
		count ++ ;
		if(count >= 10)													//连续读取失败，退出返回错误码
			return 1;
	}	
	readflag = 1;														  //重新更新标志位
	while(readflag){
		readflag = M24LC64F_ReadReg(0x00,MN,32);	 		//读取EEPROM存放的版本信息，读取长度32位
		Delay_ms(10);														//延时十毫秒，确保读取稳定
		count ++ ;
		if(count >= 10)													//连续读取失败，退出返回错误码
			return 1;
