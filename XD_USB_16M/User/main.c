...
typedef enum{
	FatFS_OK,		 // 准备完成
	FatFS_PREPARE    // 准备中
} SaveFile;			 // 数据保存到Flash状态


// 全局变量
volatile Send4GState WeatherDataState = DATA_PREPARE;						// 数据包上传标志位
volatile SaveFile  FlashSaveState   = FatFS_PREPARE;					// Flash保存标志位		
...
Time_Def AlarmTime={0x30,0x01,0x00,0x0,0,0,0};       //设置重启时间   BCD码形式  每天0点重启
positioning_ positioning; 							// 定位信息
StateOfBattrey BeforeState;


USB_OTG_CORE_HANDLE USB_OTG_dev;
uint8_t clear_count = 0;								// 降雨量每日清零 计数
uint8_t clear_rain = 0;									// 降雨量清零标志位
/*****************************************************
硬件看门狗初始化函数
配置超时时间 20s
*****************************************************/
...
int main(void)
{
  ...
  uart_init(115200);                													//4G串口初始化
	...
	M24LC64F_ReadReg(VERSION_http,(uint8_t *)HttpVersion,32);      		//读取版本信息
	sprintf(HttpCmp,"%s_01",AdressID);
	if(strcmp(HttpVersion,HttpCmp)!=0)
		M24LC64F_WriteReg(VERSION_http,(uint8_t *)HttpCmp,32);						//写入错误的处理！！！防止重复下载	
	...
	fatfs_init();																								// 挂载FATFS系统
	Fat_FilePath(Fatname.Filename,Fatname.Dirname,&RTCTime);		// 根据系统时间创造或选择目录
	Delay_ms(100);
	...
	while (1)
	{
			...
			if(WeatherDataState == DATA_READY) {   									//数据包上传标志判断
				...
				if(FlashSaveState == FatFS_OK){										//Flash保存数据标志判断
					FlashSaveState = FatFS_PREPARE;											//清空标志位
					if(StateOfHarfWare != CLOSE_ALL)										//判断RS485未关闭
						File_write(Fatname.Filename,Fatname.Dirname,&RTCTime);																									//数据写到文件系统	
					...
					if(RTCTime.hour==0){
							 clear_count = 0;																//清空读取计次
							 clear_rain = 1; 																//清除标志位置错 等待正确清除标志
							 while(clear_count <	10 && clear_rain != 0){
								 clear_rain =	RainClear_Cmd();
								 clear_count ++ ;
							 }
					}
				}
		...
		if(StateOfHarfWare != CLOSE_ALL ){																//判断状态RS485未关闭
			switch(Timecount)  																							//功能周期执行  进行定期喂狗
			{
				...
				case 40: CO2_Cmd(); IWDG_ReloadCounter(); break;		//获取风向传感器信息			风向角度	
